...
export default function OnboardingModal({ open, onComplete }: OnboardingModalProps) {
  ...
  async function handleAddProvider() {
    ...
    const presets: Record<string, { name: string; model: string; baseUrl?: string }> = {
      openai: { name: 'OpenAI', model: 'gpt-4o', baseUrl: 'https://api.openai.com/v1' },
      anthropic: { name: 'Anthropic', model: 'claude-sonnet-4', baseUrl: 'https://api.anthropic.com/v1' },
      ollama: { name: 'Ollama', model: 'llama3', baseUrl: 'http://localhost:11434/v1' }
    }
                ...
                <div className="flex flex-col gap-2">
                  {PROVIDER_TYPES.map((p) => {
                    ...
                    return (
                      <button
                        key={p.type}
                        onClick={() => { setSelectedProvider(p.type); setApiKey('') }}
