...
### 场景 4 — 内容创作：一句生成完整 Word

> 输入：根据 `~/Notes/选题.md` 写一篇 2000 字公众号文章，标题要悬念式

OpenDesk 会读 Markdown → 让 AI 起草 → 调 `desktop_type` 把内容粘贴进 Pages/Word → 在右侧 Artifact 面板里渲染 HTML 预览。
...
- **v0.6.0 — Claw 远程控制**
  - 手机扫码（微信/企业微信/Telegram/Discord）→ 远程下发指令
  - 语音输入 + Push-to-talk
  - 远程场景：通勤修 bug、机场跑报表、家里点外卖

- **v0.7.0 — 多格式导出 + RAG v2**
  - 一键导出为 Word / Excel / PPT / Markdown（对标 WorkBuddy 智能文档生成）
...
- **v0.9.0 — 团队协作**
  - Workspace 共享 + 权限分级
  - Skill Marketplace（社区上传 + 评分）
  - 多用户审计日志

- **v1.0.0 — 稳定 API + 插件市场 + 白标打包**
  - 公开插件 API，允许第三方扩展工具 / Skill
  - 白标打包（公司名 / Logo / 域名）
  - 完整 SLA + 商业支持
