...
| 模块 | 功能 | 实现路径 | 工期 | 状态 |
|------|------|---------|------|------|
| **三大工作模式** | Ask / Plan / Craft 切换器放 TopBar | 复用现有 Approval Mode（4 档 → 3 档语义化） | 3 天 | ✅ v0.6.0 |
| **七大板块导航** | 助理 / 项目 / 专家 / 自动化 / 文件 / 知识库 / 灵感 | 在 LeftSidebar 加 segmented control | 2 天 | ✅ v0.6.0 |
| **多 Agent 并行** | 多个线程同时跑，进度条独立显示 | 扩展现有 Ensemble 模式，加 Worker Pool | 5 天 | ✅ v0.6.0（runner v0.6.1） |
| **30+ 预设 Skill** | 内置从 6 个扩到 30+ | 写 24 个 `SKILL.md` 模板（含小红书运营、Excel 汇总、票据识别、销售洞察等） | 5 天 | ✅ 30/30 (v0.6.2) |
| **专家系统** | 包装 Skill 为"领域专家" | Skill + 固定 system prompt + 调用入口 | 3 天 | ✅ v0.6.0 |
| **多模态导出** | Artifacts 一键导出 Word / Excel / PPT | `docx` + `xlsx` + `pptxgenjs` | 4 天 | ✅ v0.6.0 |
| **定时任务（自动化）** | "每天 9 点自动跑日报" | `node-cron` + IPC + SQLite 持久化 | 4 天 | ✅ v0.6.0 |
| **侧边栏搜索 + 分组** | MiddleColumn 顶部加 search box | 已存在的 Thread 列表升级 | 1 天 | ✅ v0.6.0 |
| **结果区（变更记录独立面板）** | 提取 tool call 到独立 panel | 抽离现有 Message 流 | 4 天 | ✅ v0.6.0 |
| **工作流沉淀** | Skill "Save as Template" 按钮 | 在 SkillPanel 加保存路径 | 2 天 | ✅ v0.6.0 |

**v0.6.0 总工时：≈ 5–6 周（一人全职） · v0.6.2 收官：10/10 模块 ✅ + 30/30 Skill ✅**

### 📦 v0.7.0 — 生产力跃升（已发版）

- **Skills Marketplace**：8 个 entry 一键安装 + 版本锁定 + 自动更新检查
- **RAG v2**：FTS5 BM25 + TF-IDF 向量混合检索 + Reciprocal-Rank Fusion
- **Claw 远程控制**：Telegram bot（长轮询）作为首发平台，WeChat/WeCom/Lark/DingTalk 排到 v0.8.0
- **Settings · Workflow 面板**：统一管理 Scheduler / Change Log / Marketplace / Claw
